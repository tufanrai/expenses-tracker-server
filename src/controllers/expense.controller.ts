...
export const create = asyncHandler(async (req: Request, res: Response) => {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; background-color: #f9f9f9; padding: 20px; border-radius: 10px; max-width: 600px; margin: 0 auto; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1);">
      <h1 style="color: #333; font-size: 24px; margin-bottom: 20px;">New Expense Added by ${
        req.user.full_name
      }</h1>
      ...
      <div style="background-color: #fff; padding: 15px; border-radius: 8px; border: 1px solid #e0e0e0;">
        <p style="font-size: 16px; color: #555; margin: 10px 0;"><strong>Title:</strong> <span style="color: #000;">${
          expense.title
        }</span></p>
        <p style="font-size: 16px; color: #555; margin: 10px 0;"><strong>Amount:</strong> <span style="color: #2d87f0;">Rs.${
          expense.amount
        }</span></p>
        <p style="font-size: 16px; color: #555; margin: 10px 0;"><strong>Expense Date:</strong> <span style="color: #000;">${
          expense.date
        }</span></p>
...
export const update = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (removedImages && expense.receipts.length > 0) {
    ...
    expense.receipts = expense.receipts.filter(
      (receipt) => !deletedImages.includes(receipt?.public_id?.toString())
    ) as any;
  }
...
export const getAllByUser = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    const expenses = await Expense.find({ user: userId, ...filter })
      .limit(limit)
      .skip(skip)
      .sort({ createdAt: -1 })
      .populate("category");
...
export const remove = asyncHandler(async (req: Request, res: Response) => {
  ...
  if (expense.receipts && expense.receipts.length > 0) {
    const public_ids = expense.receipts.map(
